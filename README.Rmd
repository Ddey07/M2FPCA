...
knitr::opts_chunk$set(
  ...
  fig.path = "man/figures/README-", dpi = 150, fig.width = 8, out.width = "100%"
)
```
...
> Functional Data (M²FPCA).** Joint covariance estimation and FPCA for several
> functional variables of *different* types — continuous, truncated, ordinal,
> binary — measured over a common domain.

## The problem

Studies like mobile-health (mHealth) record several signals over time for each
subject, on *different measurement scales*: continuous accelerometer activity, a
binary "did you exercise?" indicator, an ordinal mood rating (0–4), a
zero-inflated (truncated) symptom count. Standard multivariate FPCA assumes all
curves are Gaussian/continuous, which none of these are. We want one joint model
that

1. estimates how each variable **co-varies over time** (marginal covariance), and
2. estimates how the variables **co-vary with each other** across time
   (cross-covariance), and
3. produces interpretable **shared temporal modes** and **subject scores** that
   borrow strength across variables.

## How M²FPCA works

M²FPCA places a **latent Gaussian copula** under the observed data: each observed
curve `X_v(t)` is a monotone (type-specific) transform of a latent Gaussian
process `Z_v(t)`. Because Kendall's τ is invariant to monotone transforms, the
*latent* correlations are identified from the observed data through type-specific
**bridging functions** — even for binary/ordinal/truncated margins.
...
2. **Cross surfaces.** For every pair, the latent cross-covariance `C_jk(s,t)`
   is estimated by fitting a tensor-product spline to the *bridged* Kendall's τ
   surface, using the correct cross-type bridge (`pfpca_crosscov`).
3. **Partial separability.** The variables are assumed to share a common
   temporal eigenbasis `{φ_ℓ(t)}` (estimated from the averaged marginal
   covariance). Cross-variable dependence is then summarised by small `p × p`
   score-covariance matrices `S_ℓ`, one per component — a compact, interpretable
   multivariate Karhunen–Loève representation.

This works for **regular** grids and **irregular / sparse / asynchronous**
sampling (the `min_no_pairs` cutoff drops time-point pairs with too few
co-observations before fitting).
...
sharing a partially separable latent structure, then check that M²FPCA recovers
the truth from the mixed-type observations.
...
The latent target is a correlation surface. Analytically the true full latent
correlation is `Rcross %x% cov2cor(K)` with `K = sum_l lambda_l phi_l phi_l'`.
...
# cov.from.mpfpca.dir() returns the reconstructed covariance (its diagonal is the
# estimated variance surface, ~1 but not exactly 1); normalise to a correlation
# so it is on the same scale as R_true and has a unit diagonal.
R_hat  <- cov2cor(cov.from.mpfpca.dir(fit))   # estimated (mp x mp) latent correlation
```

```{r cov-heatmaps, fig.height = 3.4}
...
Each `m × m` block is one variable pair: the three diagonal blocks are the
marginal correlation surfaces, the off-diagonal blocks the cross-correlations.
M²FPCA recovers all of them from continuous + binary + ordinal data.

### Shared temporal eigenfunctions

```{r eigfun, fig.height = 3.4}
...
Subject scores from `mpfpca_scores` recover the cross-variable correlation
structure (true off-diagonal = 0.5):
...
Pass data with `NA`s; `min_no_pairs` controls the reliability cutoff (time-point
pairs with fewer co-observations are dropped before fitting):
...
`mpfpca.dir()` returns: `pfpca_results` (per-variable `lambda`/`phi`/`cumFVE`),
`S` (per-component `p × p` score covariances), `L`, `raw_est` (raw block
covariance), `raw_est_cov` (its delta-method variance), and `cov_marginal`.
...
Application to Mood Disorders.* **arXiv preprint arXiv:2603.11385**.
[arXiv:2603.11385](https://arxiv.org/abs/2603.11385)

The single-variable estimator this package builds on is described in Dey, D.,
Ghosal, R., Merikangas, K., & Zipunnikov, V. (2024). *Functional Principal
...
See [`BUILD_NOTES.md`](BUILD_NOTES.md) for how the package was assembled and
validated against the original research code (including the fgm fidelity check).
